  ...
  useEffect(() => {
    const fetchActiveMatch = async () => {
      try {
        ...
        if (response.success) {
          setGameData({gameId: response.data.game_id, status: response.data.status})
          let parsedPattern = [];
          try {
            parsedPattern =
          ...
          // activeNumbers: last with true, others with false
          const newActiveNumbers = {};
          parsedCalled.forEach((n) => {
            newActiveNumbers[String(n)] = true;
          });

          // newlyCalled: compare with prev allNumbers to view news numbers
          ...
          console.error("Error fetching active match:", response.message);
        }
      } catch (error) {
