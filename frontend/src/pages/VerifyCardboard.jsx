...
export default function VerifyCardboard() {
  ...
  // Efecto para filtrar cuando cambia searchTerm o groupedCards
  useEffect(() => {
    if (!searchTerm) {
    ...
    const filtered = Object.entries(groupedCards).reduce(
      (acc, [userId, userData]) => {
        // Buscar coincidencias en teléfono o nombre
    ...
    <section className="max-w-[768px] mx-auto h-full flex flex-col items-center justify-center pt-4">
      ...
      >
        {message ? (
          ...
          <p className="mt-42 flex items-center mx-auto w-full text-center text-[#FAF2E7] font-semibold font-poppins">
            No hay cartones reservados en estos momentos
          </p>
        ) : (
