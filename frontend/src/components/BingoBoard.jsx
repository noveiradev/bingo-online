export default function BingoBoard({ BINGO_COLUMNS, activeNumbers, newlyCalled, lastCalled }) {
  return (
        ...
        <div key={col.letter} className="border border-white/10">
          {Array.from({ length: 15 }, (_, i) => {
            ...
            return (
              <div
                key={number}
                className={`h-[1.85rem] desk:h-[1.47rem] desklg:h-[1.78rem] flex mb-[0.10rem] font-semibold font-inter items-center justify-center transition-all duration-200
                  ${
                    isLastCalled
